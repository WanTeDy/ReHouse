...
{
    public class DeleteCategoriesOp : BaseOperation
    {
        ...
        public List<BrainCategory> Recurs(List<BrainCategory> outCategory, IQueryable<BrainCategory> sourceCategory)
        {
            foreach (var categoriesModel in outCategory)
            {
                ...
                if (addCategories.Any())
                {
                    ...
                    {
                        if (brainCategory.BrainProducts.Any())// && !brainCategory.Categories.Any()) 
                        {
                            ...
                            brainCategory.Deleted = false;
                        }
                        else if (brainCategory.Categories.Any())
                        {
                            categoriesModel.Categories.Add(new BrainCategory
                            {
                                Id = brainCategory.Id,
        ...
        protected override void InTransaction()
        {
            ...
            var categories = Context.Categories.Where(x => x.parentID != 1 && x.parentID != 0);
            ...
            var newEl = hierarchy.ToList().Select(brainCategory => new BrainCategory
            {
                Id = brainCategory.Id,
                BrainParentID = brainCategory.BrainParentID,
                categoryID = brainCategory.categoryID,
